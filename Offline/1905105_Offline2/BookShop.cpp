	...
	public:
		...
		/*Write necessary code for the copy constructor*/
            ...
            for(int i=0;i<count;i++)
                *(books+i) = *(bs.books+i);
           // cout << "Constructing cpy" <<endl;
        }

		~BookShop(){//Destructor
		/*Free memory as applicable*/
            delete []books;
           // cout << "Destructing size:" << size <<endl;
		}
		...
		/* Use the title **MergedShop** of the new bookshop*/
            BookShop MergedShop;
            MergedShop.setName("MergedShop");
            MergedShop.size = this -> size + b.size;
            MergedShop.addBook(this -> books, this -> count);
